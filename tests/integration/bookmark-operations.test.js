...
test.describe('ForgetfulMe Bookmark Operations Tests', () => {
  ...
  test('should edit existing bookmark', async ({ page }) => {
    ...
    const editButton = await page
      .locator('.edit-bookmark, button[title*="edit"]')
      .first();
  ...
  test('should delete bookmark with confirmation', async ({ page }) => {
    ...
    const deleteButton = await page
      .locator('.delete-bookmark, button[title*="delete"]')
      .first();
    if (await deleteButton.isVisible()) {
      ...
      if (await confirmDialog.isVisible()) {
        ...
        await expect(successMessage).toBeVisible({
          timeout: 5000,
          message:
            'Success message should be displayed after bookmark deletion',
        });
      }
    }
  });

  test('should handle bookmark operations errors gracefully', async ({
    page,
  }) => {
    // Set up authenticated state
    await extensionHelper.setupAuthenticatedState();
