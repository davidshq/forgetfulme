...
test.describe('ForgetfulMe Accessibility and Keyboard Navigation', () => {
  ...
  test('Popup keyboard navigation workflow', async () => {
    ...
    for (let i = 0; i < 20; i++) {
      // Maximum 20 tab stops to prevent infinite loop
  ...
  test('Options page keyboard navigation workflow', async () => {
    ...
    const formInputs = await optionsPage
  ...
  test('ARIA attributes and screen reader compatibility', async () => {
    ...
    for (const input of inputs) {
      ...
      if (inputId) {
        ...
        const associatedLabel = await popupPage.locator(
          `label[for="${inputId}"]`
        );
  ...
  test('Keyboard shortcuts accessibility', async () => {
    ...
    const shortcutDetected = await testPage.evaluate(
      () => window.accessibilityShortcutDetected
    );
    ...
    const focusTest = await popupPage.evaluate(() => {
      ...
      return {
        ...
        isInteractive: ['BUTTON', 'INPUT', 'SELECT', 'TEXTAREA', 'A'].includes(
  ...
  test('Focus management and visual indicators', async () => {
    ...
    const interactiveElements = await optionsPage
      .locator('button, input, select, a, [tabindex]')
      .all();
    ...
    for (let i = 0; i < Math.min(interactiveElements.length, 5); i++) {
      ...
      if (!hasFocusIndicator) {
        console.warn(
          'Element may lack focus indicator:',
          await element.tagName()
        );
      }
  ...
  test('Error message accessibility', async () => {
    ...
    if ((await actionButton.count()) > 0) {
      ...
      const errorElements = await popupPage
        .locator('.ui-message.error, .error-message, [role="alert"]')
        .all();
