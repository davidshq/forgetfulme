...
test.describe('ForgetfulMe Complete User Workflows', () => {
  ...
  test('First-time user setup workflow', async () => {
    ...
    const hasSetupContent = pageContent.toLowerCase().includes('welcome') || 
                           pageContent.toLowerCase().includes('setup') ||
                           pageContent.toLowerCase().includes('configure');
    
    console.log('Popup content type:', hasSetupContent ? 'Setup interface' : 'Main interface');
    
    // Step 2: Test settings navigation (if setup interface exists)
    const settingsButtons = await popupPage.locator('button').filter({ hasText: /settings|configure|setup|⚙️/i }).count();
    console.log('Settings buttons found:', settingsButtons);
    
    if (settingsButtons > 0) {
      // There are settings-related buttons
      const settingsButton = popupPage.locator('button').filter({ hasText: /settings|configure|setup|⚙️/i }).first();
    ...
    const inputElements = await optionsPage.locator('input, select, textarea').count();
  ...
  test('Bookmark management workflow', async () => {
    ...
    const bookmarkInterface = bookmarkPage.locator('.bookmark-list, .search-card, .bookmark-management');
    ...
    if (await bookmarkInterface.count() > 0) {
      ...
      const addButton = bookmarkPage.locator('button').filter({ hasText: /add|new|create/i });
      if (await addButton.count() > 0) {
        await addButton.click();
        
        // Fill new bookmark form
        const urlInput = bookmarkPage.locator('#bookmarkUrl, input[name="url"]');
        const titleInput = bookmarkPage.locator('#bookmarkTitle, input[name="title"]');
        ...
        if (await urlInput.count() > 0 && await titleInput.count() > 0) {
          ...
          const saveButton = bookmarkPage.locator('button').filter({ hasText: /save|create/i });
  ...
  test('Settings and preferences workflow', async () => {
    ...
    const statusSection = optionsPage.locator('.custom-status, .status-types, #customStatusTypes');
    
    if (await statusSection.count() > 0) {
      // User adds new status type
      const addStatusButton = optionsPage.locator('button').filter({ hasText: /add.*status|new.*status/i });
      
      if (await addStatusButton.count() > 0) {
        await addStatusButton.click();
        
        const statusInput = optionsPage.locator('input[placeholder*="status"], input[name*="status"]').last();
        if (await statusInput.count() > 0) {
          await statusInput.fill('research-later');
          
          const saveButton = optionsPage.locator('button').filter({ hasText: /save|apply/i });
          if (await saveButton.count() > 0) {
            await saveButton.click();
    ...
    const preferencesSection = optionsPage.locator('.preferences, .user-preferences');
    ...
    const saveAllButton = optionsPage.locator('button').filter({ hasText: /save all|apply changes/i });
  ...
  test('Error handling and recovery workflow', async () => {
    ...
    const markButton = popupPage.locator('button').filter({ hasText: /mark|save|bookmark/i });
    if (await markButton.count() > 0) {
      await markButton.click();
      await popupPage.waitForTimeout(3000);
      
      // Should see error message
      const errorMessage = popupPage.locator('.ui-message.error, .error-message, .network-error');
