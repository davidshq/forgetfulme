...
test.describe('ForgetfulMe Cross-Device Synchronization', () => {
  ...
  test('Multi-context extension loading simulation', async () => {
    ...
    const device1IsValid =
      device1URL.includes('chrome-extension://') ||
      device1URL.includes('localhost:3000');
    const device2IsValid =
      device2URL.includes('chrome-extension://') ||
      device2URL.includes('localhost:3000');
  ...
  test('Bookmark creation workflow across multiple contexts', async () => {
    ...
    const urlInput = device1Options.locator(
      '#supabaseUrl, input[name="supabaseUrl"]'
    );
    const keyInput = device1Options.locator(
      '#supabaseKey, input[name="supabaseKey"]'
    );
    ...
    if ((await urlInput.isVisible()) && (await keyInput.isVisible())) {
      ...
      const saveButton = device1Options
        .locator('button')
    ...
    const titleInput = device1Popup.locator(
      '#bookmarkTitle, input[name="title"]'
    );
    const markButton = device1Popup
      .locator('button')
    ...
    const searchInput = device2Bookmark.locator(
  ...
  test('Real-time update simulation workflow', async () => {
    ...
    const addButton = device1Bookmark
      .locator('button')
    ...
    if ((await addButton.count()) > 0) {
      ...
      const urlInput = device1Bookmark.locator(
        '#bookmarkUrl, input[name="url"]'
      );
      const titleInput = device1Bookmark.locator(
        '#bookmarkTitle, input[name="title"]'
      );
      const saveButton = device1Bookmark
        .locator('button')
    ...
    const refreshButton = device2Bookmark
      .locator('button')
  ...
  test('Concurrent user actions across contexts', async () => {
    ...
    const device1Title = device1Popup.locator(
      '#bookmarkTitle, input[name="title"]'
    );
    ...
    const device2StatusInput = device2Options.locator(
      'input[name*="status"], #customStatusTypes'
    );
    ...
    const [device1Save, device2Save] = await Promise.allSettled([
      (async () => {
        const saveBtn = device1Popup
          .locator('button')
          .filter({ hasText: /save|mark/i });
  ...
  test('Session state isolation between contexts', async () => {
    ...
    const device1Toggle = device1Options
    ...
    const device2Toggle = device2Options
      .locator('input[type="checkbox"]')
      .first();
  ...
  test('Error handling across multiple contexts', async () => {
    ...
    const device1SaveBtn = device1Popup
      .locator('button')
      .filter({ hasText: /save|mark/i });
    if ((await device1SaveBtn.count()) > 0) {
      ...
      const errorMessage = device1Popup.locator(
    ...
    const device2SaveBtn = device2Popup
      .locator('button')
      .filter({ hasText: /save|mark/i });
