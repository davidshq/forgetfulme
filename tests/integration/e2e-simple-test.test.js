...
test.describe('Simple Extension Loading Test', () => {
  test('should validate Chrome browser environment and extension capabilities', async ({
    page,
    context,
  }) => {
    ...
    await page.goto(
      'data:text/html,<html><head><title>Extension Test Page</title></head><body><h1>Testing Extension Environment</h1><p>This page tests Chrome extension capabilities.</p></body></html>'
    );
    ...
    const popupTest = await page.evaluate(() => {
      ...
      popup.innerHTML =
        '<h2>Extension Popup Test</h2><button id="test-button">Test Button</button>';
      popup.style.cssText =
        'position: fixed; top: 10px; right: 10px; background: white; border: 1px solid #ccc; padding: 10px; z-index: 9999;';
