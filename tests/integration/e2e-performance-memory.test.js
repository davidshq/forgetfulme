...
test.describe('ForgetfulMe Performance and Memory', () => {
  ...
  test('Options page load time performance', async () => {
    ...
    if (loadTime > 2000) {
      console.warn(
        `Options page load time: ${loadTime}ms (consider optimization)`
      );
    }
  ...
  test('Bookmark management page load time', async () => {
    ...
    if (loadTime > 3000) {
      console.warn(
        `Bookmark management load time: ${loadTime}ms (consider optimization)`
      );
    }
  ...
  test('Memory usage with multiple page opens', async () => {
    ...
    if (initialMemory && finalMemory) {
      ...
      console.log(
        `Memory increase after multiple page operations: ${memoryIncreaseMB.toFixed(2)}MB`
      );
      ...
      if (memoryIncreaseMB > 5) {
        console.warn(
          `Significant memory increase detected: ${memoryIncreaseMB.toFixed(2)}MB`
        );
      }
    }
  });
  ...
  test('Extension responsiveness under load', async () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (buttonCount > 0) {
        const randomButton = buttons.nth(
  ...
  test('Error recovery performance', async () => {
    ...
    const actionButton = popupPage
      .locator('button')
      .filter({ hasText: /save|mark|bookmark/i });
  ...
  test('Multiple page instances performance', async () => {
    ...
    try {
      ...
      const workingPages = results.filter(
        result => result.status === 'fulfilled' && result.value
      ).length;
      expect(workingPages).toBeGreaterThan(0);

      console.log(
        `Multi-page health check time: ${interactionTime}ms, working pages: ${workingPages}`
      );
    } catch (error) {
      ...
      try {
        ...
        if (bookmarkPage && !bookmarkPage.isClosed())
          await bookmarkPage.close();
      } catch (cleanupError) {
  ...
  test('Long session stability', async () => {
    ...
    console.log(
      `Session stability test completed: ${activities.length} activities, avg time: ${avgActivityTime.toFixed(2)}ms`
    );
