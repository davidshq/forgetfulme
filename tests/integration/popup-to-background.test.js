...
test.describe('ForgetfulMe Popup to Background Communication Tests', () => {
  ...
  test('should send bookmark saved message to background', async ({ page }) => {
    ...
    const titleInput = await page.locator('#bookmarkTitle, input[name="title"]');
  ...
  test('should handle background message responses', async ({ page }) => {
    ...
    const responseMessage = await page.locator('.ui-message.success, .response-message');
    await expect(responseMessage).toBeVisible({ 
      timeout: 5000,
      message: 'Response message should be displayed after background communication'
    });
  });
  ...
  test('should handle background message errors', async ({ page }) => {
    ...
    await expect(errorMessage).toBeVisible({ 
      timeout: 5000,
      message: 'Error message should be displayed after background communication error'
    });
  });

  test('should sync auth state between popup and background', async ({ page }) => {
    // Set up authenticated state
    await extensionHelper.setupAuthenticatedState();
    ...
    const unauthenticatedIndicator = await page.locator('.auth-container, .sign-in-prompt');
    await expect(unauthenticatedIndicator).toBeVisible({ 
      timeout: 5000,
      message: 'Unauthenticated state should be visible after auth state change'
    });
  });
  ...
  test('should handle keyboard shortcuts from background', async ({ page }) => {
    ...
    const statusIndicator = await page.locator('.status-read, .bookmark-status');
