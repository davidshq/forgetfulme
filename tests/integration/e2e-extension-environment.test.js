...
test.describe('ForgetfulMe Real Extension Environment', () => {
  ...
  test('should open popup with extension or fallback URL', async () => {
    ...
    const isValidURL =
      popupURL.includes(`chrome-extension://${extensionId}/popup.html`) ||
  ...
  test('should open options page with extension or fallback URL', async () => {
    ...
    const isValidURL =
      optionsURL.includes(`chrome-extension://${extensionId}/options.html`) ||
  ...
  test('should open bookmark management page with extension or fallback URL', async () => {
    ...
    const isValidURL =
      bookmarkURL.includes(
        `chrome-extension://${extensionId}/bookmark-management.html`
  ...
  test('should have proper Chrome extension APIs available or working fallbacks', async () => {
    ...
    expect(apiAvailability.hasChrome || apiAvailability.hasMockChrome).toBe(
      true
    );
    ...
    if (apiAvailability.hasChrome) {
      ...
      const availableApiCount = Object.values(apiAvailability.apis).filter(
        Boolean
      ).length;
  ...
  test('should respond to keyboard shortcuts or simulate shortcut behavior', async ({
    page,
  }) => {
  ...
  test('should maintain consistent interface between popup opens', async () => {
    ...
    expect(
      content1.includes('ForgetfulMe') || content2.includes('ForgetfulMe')
    ).toBeTruthy();
      ...
      return new Promise(resolve => {
        if (
          ...
          resolve({
            received: true,
            source: 'fallback',
            message: 'Chrome runtime not available in test environment',
          });
        }
      });
    });
  ...
  test('should navigate between extension pages correctly', async () => {
    ...
    const isPopupValid =
      popupURL.includes(`chrome-extension://${extensionId}`) ||
      popupURL.includes('localhost:3000');
    const isOptionsValid =
      optionsURL.includes(`chrome-extension://${extensionId}`) ||
      optionsURL.includes('localhost:3000');
    const isBookmarkValid =
      bookmarkURL.includes(`chrome-extension://${extensionId}`) ||
      bookmarkURL.includes('localhost:3000');
