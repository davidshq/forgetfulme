...
8. **Create dedicated test files for shared utilities** (e.g., `formatters.test.js` for formatting utilities)
9. **Avoid duplicating tests** - if a utility is tested in its own file, don't duplicate those tests in component test files
...
6. **Don't duplicate tests for shared utilities** - test shared utilities in their dedicated test files, not in every component that uses them
