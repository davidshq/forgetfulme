...
class AuthStateManager {
  ...
  async initialize() {
    ...
    try {
      ...
      console.log('AuthStateManager initialized, current state:', this.authState ? 'authenticated' : 'not authenticated')
    } catch (error) {
  ...
  async setAuthState(session) {
    ...
    console.log('Auth state updated:', session ? 'authenticated' : 'not authenticated')
  }
  ...
  handleAuthStateChange(newAuthState) {
    ...
    if (oldAuthState !== newAuthState) {
      ...
      console.log('Auth state changed via storage:', newAuthState ? 'authenticated' : 'not authenticated')
    }
  }
